...
extension ByteBuffer {
    ...
    public mutating func readInteger<T: FixedWidthInteger>(
        endianness: Endianness = .big, as: T.Type = T.self
    ) -> T? {
        guard let result = self.getInteger(at: self.readerIndex, endianness: endianness, as: T.self)
        else {
            return nil
        }
    ...
    public func getInteger<T: FixedWidthInteger>(
        ...
        guard let range = self.rangeWithinReadableBytes(index: index, length: MemoryLayout<T>.size)
        else {
            return nil
        }
...
extension FixedWidthInteger {
    ...
    @inlinable
    init?(buffer: ByteBuffer, endianness: Endianness = .host) {
        var buffer = buffer
        guard let value = buffer.readInteger(endianness: endianness, as: Self.self),
