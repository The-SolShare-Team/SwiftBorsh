...
@usableFromInline let sysRealloc:
...
public struct ByteBufferAllocator: Sendable {
    ...
    internal init(
        ...
        hookedRealloc:
            @escaping @convention(c) (UnsafeMutableRawPointer?, size_t) -> UnsafeMutableRawPointer?,
        hookedFree: @escaping @convention(c) (UnsafeMutableRawPointer) -> Void,
        hookedMemcpy:
            @escaping @convention(c) (UnsafeMutableRawPointer, UnsafeRawPointer, size_t) -> Void
    ) {
    ...
    @usableFromInline internal let memcpy:
        @convention(c) (UnsafeMutableRawPointer, UnsafeRawPointer, size_t) -> Void
}
...
public struct ByteBuffer {
    ...
    @usableFromInline final class _Storage {
        ...
        @inlinable
        init(
            bytesNoCopy: UnsafeMutableRawPointer, capacity: _Capacity,
            allocator: ByteBufferAllocator
        ) {
        ...
        func allocateStorage(capacity: _Capacity) -> _Storage {
            ...
            return _Storage(
                bytesNoCopy: _Storage._allocateAndPrepareRawMemory(
                    bytes: newCapacity, allocator: self.allocator),
        ...
        func reallocSlice(_ slice: Range<ByteBuffer._Index>, capacity: _Capacity) -> _Storage {
            ...
            self.allocator.memcpy(
                new.bytes, self.bytes.advanced(by: Int(slice.lowerBound)), size_t(slice.count))
            return new
        }
        ...
        func dumpBytes(slice: Slice, offset: Int, length: Int) -> String {
            ...
            for byte in bytes[
                Int(slice.lowerBound) + offset..<Int(slice.lowerBound) + offset + length]
            {
    ...
    mutating func _copyStorageAndRebase(extraCapacity: _Capacity = 0, resetIndices: Bool = false) {
        self._copyStorageAndRebase(
            capacity: _toCapacity(self._slice.count) + extraCapacity, resetIndices: resetIndices)
    }
    ...
    mutating func _ensureAvailableCapacity(_ capacity: _Capacity, at index: _Index) {
        ...
        assert(
            self._slice.lowerBound >= 0,
            "illegal slice: negative lower bound: \(self._slice.lowerBound)")
        assert(
    ...
    mutating func _setBytes(_ bytes: UnsafeRawBufferPointer, at index: _Index) -> _Capacity {
        ...
        if !isKnownUniquelyReferenced(&self._storage) {
            let extraCapacity =
                newEndIndex > self._slice.upperBound ? newEndIndex - self._slice.upperBound : 0
    ...
    @inlinable
    mutating func _setBytesAssumingUniqueBufferAccess(
        _ bytes: UnsafeRawBufferPointer, at index: _Index
    ) {
    ...
    where Bytes.Element == UInt8 {
        ...
        if !isKnownUniquelyReferenced(&self._storage) {
            let extraCapacity =
                newPastEndIndex > self._slice.upperBound
                ? newPastEndIndex - self._slice.upperBound : 0
            self._copyStorageAndRebase(extraCapacity: extraCapacity)
        }

        var base = ensureCapacityAndReturnStorageBase(capacity: underestimatedByteCount)
        var (iterator, idx) = UnsafeMutableBufferPointer(
            start: base, count: underestimatedByteCount
        ).initialize(
            from: bytes
        )
    ...
    @inlinable init(allocator: ByteBufferAllocator, startingCapacity: Int) {
        ...
        self._readerIndex = 0
        self._writerIndex = 0
        self._storage = _Storage.reallocated(
            minimumCapacity: startingCapacity, allocator: allocator)
        self._slice = self._storage.fullSlice
    }
    ...
    /// - Returns: The value returned by `body`.
    @inlinable
    public func withUnsafeReadableBytes<T>(_ body: (UnsafeRawBufferPointer) throws -> T) rethrows
        -> T
    {
    ...
    @discardableResult public mutating func discardReadBytes() -> Bool {
        ...
        if isKnownUniquelyReferenced(&self._storage) {
            self._storage.bytes.advanced(by: Int(self._slice.lowerBound))
                .copyMemory(
                    from: self._storage.bytes.advanced(
                        by: Int(self._slice.lowerBound + self._readerIndex)),
    ...
    public mutating func clear(minimumCapacity: Int) {
        ...
        precondition(
            minimumCapacity <= _Capacity.max, "Minimum capacity must be <= \(_Capacity.max)")
...
extension ByteBuffer {
    ...
    @discardableResult
    @inlinable
    public mutating func setBytes<Bytes: Sequence>(_ bytes: Bytes, at index: Int) -> Int
    where Bytes.Element == UInt8 {
...
extension ByteBuffer {
    ...
    public mutating func copyBytes(at fromIndex: Int, to toIndex: Int, length: Int) throws -> Int {
        ...
        self.withVeryUnsafeMutableBytes { ptr in
            let srcPtr = UnsafeRawBufferPointer(
                start: ptr.baseAddress!.advanced(by: fromIndex), count: length)
    ...
    public struct CopyBytesError: Error {
        ...
        public static let unreadableSourceBytes: CopyBytesError = .init(
            baseError: .unreadableSourceBytes)
    }
}
...
extension ByteBuffer {
    ...
    @inlinable
    public mutating func modifyIfUniquelyOwned<T>(_ body: (inout ByteBuffer) throws -> T) rethrows
        -> T?
    {
        if isKnownUniquelyReferenced(&self._storage) {
