...
extension ByteBuffer {
    public struct LengthPrefixError: Swift.Error {
        ...
        public static let messageLengthDoesNotFitExactlyIntoRequiredIntegerFormat:
            LengthPrefixError = .init(
...
extension ByteBuffer {
    ...
    public mutating func writeLengthPrefixed<Integer>(
        ...
        self.setInteger(
            lengthPrefix, at: lengthPrefixIndex, endianness: endianness, as: Integer.self)
...
extension ByteBuffer {
    ...
    public mutating func readLengthPrefixed<Integer, Result>(
        ...
        guard let buffer = self.readLengthPrefixedSlice(endianness: endianness, as: Integer.self)
        else {
            return nil
        }
    ...
    public mutating func readLengthPrefixedSlice<Integer>(
        ...
        guard
            let result = self.getLengthPrefixedSlice(
                at: self.readerIndex, endianness: endianness, as: Integer.self)
        else {
            return nil
        }
    ...
    public func getLengthPrefixedSlice<Integer>(
        ...
        guard
            let lengthPrefix = self.getInteger(at: index, endianness: endianness, as: Integer.self),
            let messageLength = Int(exactly: lengthPrefix),
            let messageBuffer = self.getSlice(
                at: index + MemoryLayout<Integer>.size, length: messageLength)
        else {
            return nil
        }
