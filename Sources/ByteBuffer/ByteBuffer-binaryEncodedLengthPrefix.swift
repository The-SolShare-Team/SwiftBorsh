...
extension ByteBuffer {
    ...
    @inlinable
    public mutating func readEncodedInteger<
        Strategy: NIOBinaryIntegerEncodingStrategy, Integer: FixedWidthInteger
    >(
    ...
    public mutating func writeLengthPrefixed<Strategy: NIOBinaryIntegerEncodingStrategy>(
        ...
        case reservedCapacity:
            ...
            self._moveBytes(
                from: originalDataEndIndex, to: lengthPrefixIndex, size: actualIntegerLength)
    ...
    public mutating func readLengthPrefixedSlice<Strategy: NIOBinaryIntegerEncodingStrategy>(
        ...
        guard let length = strategy.readInteger(as: Int.self, from: &self),
            let slice = self.readSlice(length: length)
        else {
...
extension ByteBuffer {
    ...
    >(
        ...
        self.reserveCapacity(
            minimumWritableBytes: buffer.readableBytes + strategy.requiredBytesHint)
        var written = 0
...
extension ByteBuffer {
    ...
    mutating func _moveBytes(from source: Int, to destination: Int, size: Int) {
        precondition(
            source >= self.readerIndex && destination < self.writerIndex && source >= destination)
