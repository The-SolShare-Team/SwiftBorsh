...
extension String {
    ...
    @inlinable
    init<Buffer: Collection>(_base64Encoding bytes: Buffer) where Buffer.Element == UInt8 {
        self = Base64.encode(bytes: bytes)
    }

    @inlinable
    func _base64Decoded() throws -> [UInt8] {
...
extension String {
    ...
    init(
        ...
        initializingUTF8With initializer: (_ buffer: UnsafeMutableBufferPointer<UInt8>) throws ->
            Int
    ) rethrows {
...
extension String {
    ...
    init(
        ...
        if #available(macOS 11.0, iOS 14.0, tvOS 14.0, watchOS 7.0, *) {
            ...
            try self.init(
                backportUnsafeUninitializedCapacity: capacity, initializingUTF8With: initializer)
        }
    }
}
