...
extension ByteBuffer {
    ...
    public mutating func readBytes(length: Int) -> [UInt8]? {
        ...
        >(
            ...
            let inlineArray = InlineArray<count, IntegerType> {
                (outputSpan: inout OutputSpan<IntegerType>) in
    ...
    @discardableResult
    @inlinable
    public mutating func readWithUnsafeReadableBytes(_ body: (UnsafeRawBufferPointer) throws -> Int)
        rethrows -> Int
    {
...
extension ByteBuffer {
    ...
    ///         accounting and optimisations of resources acquired for operations on a given `Channel` in the future.
    @inlinable
    public init<I: FixedWidthInteger>(
        integer: I, endianness: Endianness = .big, as: I.Type = I.self
    ) {
...
extension ByteBuffer {
    ...
    public mutating func readUTF8ValidatedString(length: Int) throws -> String? {
        guard let result = try self.getUTF8ValidatedString(at: self.readerIndex, length: length)
        else {
            return nil
        }
