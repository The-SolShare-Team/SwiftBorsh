...
extension ByteBufferView: RangeReplaceableCollection {
    ...
    @inlinable
    public mutating func append<Bytes: Sequence>(contentsOf bytes: Bytes)
    ...
    where ByteBufferView.Element == C.Element {
        ...
        if newElements.count == subrange.count {
            ...
            self._range =
                self._range.startIndex..<self._range.endIndex.advanced(by: additionalByteCount)
        }
    }
}
...
extension ByteBufferView: Equatable {
    ...
    public static func == (lhs: ByteBufferView, rhs: ByteBufferView) -> Bool {
        ...
        let leftBufferSlice = lhs._buffer.getSlice(
            at: lhs._range.startIndex, length: lhs._range.count)!
        let rightBufferSlice = rhs._buffer.getSlice(
            at: rhs._range.startIndex, length: rhs._range.count)!
...
extension ByteBufferView: Hashable {
    ...
    public func hash(into hasher: inout Hasher) {
        ...
        hasher.combine(
            self._buffer.getSlice(at: self._range.startIndex, length: self._range.count)!)
    }
}
